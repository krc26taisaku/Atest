    ...
    items.slice(0,100).forEach((item,index)=>{
      ...
      card.innerHTML=`
        <div class="ranking-top">
          <span class="ranking-name">${index+1}. ${escapeHtml(item.q.displayWord||item.q.word)}</span>
        ...
        ${item.q.abbreviationDetails?`<div class="abbreviation-details">${escapeHtml(item.q.abbreviationDetails)}</div>`:''}
