...
window.KenteiWordList=(()=>{
  ...
  function render(){
    ...
    let items=Q().filter(q=>{
      ...
      if(query&&!String(q.searchText||`${q.word} ${q.meaning}`).toLowerCase().includes(query))return false;
    ...
    items.forEach(q=>{
      ...
      main.innerHTML=`<span><span class="word-list-name">${escapeHtml(q.displayWord||q.word)}</span><br><span class="word-list-category">${escapeHtml(q.category)}</span></span><span>›</span>`;
      ...
      meaning.innerHTML=`
        ${q.abbreviationDetails?`<div class="abbreviation-details">${escapeHtml(q.abbreviationDetails)}</div>`:''}
        <div>${escapeHtml(q.meaning)}</div>
      `;
